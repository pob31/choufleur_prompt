...
async fn open(
    ...
    // rule, and here it also keeps the spawn and the wait properly separated.
    {
        ...
        cmd.arg("serve")
            ...
            // Our own port, so the show page can hand out an address that outlives it.
            //
            // The child's port is chosen at open time and slides upward; ours never
            // moves. Only this end knows it, and without it the page would have to
            // offer its own address — which works tonight and is a bad bookmark.
            .env("CHOUFLEUR_UI_PORT", ui.port.to_string());
        if req.prep {
            cmd.arg("--prep");
        }
        if req.live {
...
pub fn run(root: PathBuf, port: u16, admin_from_anywhere: bool) -> Result<()> {
    ...
    rt.block_on(async move {
        ...
        for a in crate::join::lan_addresses(port) {
