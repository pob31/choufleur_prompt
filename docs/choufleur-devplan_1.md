...
*Built in Phase 0:* flags are attributed and time-stamped (`{by, at}`, an array — two
lists may mark the same line). `by` is the **cue list's name**, not an operator's, which
removes the dependency on Pre-Coding Decision 5 entirely: a list is a position, a screen
already knows which one it is showing, and nothing is authorised by the name.
